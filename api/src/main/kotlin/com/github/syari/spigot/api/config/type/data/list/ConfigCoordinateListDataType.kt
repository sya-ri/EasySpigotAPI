...
object ConfigCoordinateListDataType : ConfigDataType<List<Coordinate>> {
    ...
    override fun get(
        ...
        return config.get(path, ConfigDataType.StringList, notFoundError)?.mapNotNull {
            ConfigCoordinateDataType.stringToCoordinate(config, "$path.$it", it)
        }.orEmpty()
    }
    ...
    override fun set(
        ...
        config.set(path, ConfigDataType.StringList, value?.map(ConfigCoordinateDataType::coordinateToString))
    }
}
